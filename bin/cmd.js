...
const argv = minimist(process.argv.slice(2), {
  ...
  boolean: ['version', 'verbose', 'json', 'nm-reconnect', 'force', 'help', 'update']
})
const cmd = argv._[0]
...
let config = null
const configPath = utils.resolveStatePath(
  path.join(os.homedir(), '.spoofdrc'),
  path.join(os.homedir(), '.spoofyrc')
)
...
function warnIfNotElevated () {
  ...
  console.error(chalk.yellow('⚠'), chalk.gray('MAC changes usually require root. If this fails, try: sudo spoofd <command>'))
}
...
function installCompletions () {
  ...
  if (shell.includes('fish')) {
    targetShell = 'fish'
    completionFile = path.join(completionsDir, 'spoofd.fish')
    if (platform === 'win32') {
      installPath = path.join(os.homedir(), '.config', 'fish', 'completions', 'spoofd.fish')
    } else {
      installPath = path.join(os.homedir(), '.config', 'fish', 'completions', 'spoofd.fish')
    }
  } else if (shell.includes('zsh')) {
    targetShell = 'zsh'
    completionFile = path.join(completionsDir, 'spoofd.zsh')
    if (platform === 'win32') {
      installPath = path.join(os.homedir(), '.zshrc.d', '_spoofd')
    } else {
      installPath = path.join(os.homedir(), '.zshrc.d', '_spoofd')
    }
  } else if (shell.includes('bash') || shell.includes('sh')) {
    targetShell = 'bash'
    completionFile = path.join(completionsDir, 'spoofd.bash')
    if (platform === 'win32') {
      installPath = path.join(os.homedir(), '.bash_completion.d', 'spoofd')
    } else {
      installPath = path.join(os.homedir(), '.bash_completion.d', 'spoofd')
    }
  } else if (platform === 'win32') {
    targetShell = 'powershell'
    completionFile = path.join(completionsDir, 'spoofd.ps1')
    installPath = path.join(os.homedir(), 'Documents', 'PowerShell', 'spoofd.ps1')
  }

  if (!targetShell) {
    console.error(chalk.red('✗'), 'Could not detect shell. Please specify with --shell=<bash|zsh|fish|powershell>')
    console.log(chalk.yellow('\nManual installation:'))
    console.log(chalk.gray('  Bash:  source ' + path.join(completionsDir, 'spoofd.bash')))
    console.log(chalk.gray('  Zsh:   Add to .zshrc: fpath=(' + completionsDir + ' $fpath)'))
    console.log(chalk.gray('  Fish:  Copy to ~/.config/fish/completions/spoofd.fish'))
    console.log(chalk.gray('  PS:    Add to $PROFILE: . ' + path.join(completionsDir, 'spoofd.ps1')))
    process.exit(1)
  }
  ...
  if (err.code) {
    ...
    if (err.message.includes('ENOENT') || err.message.includes('spawn')) {
      ...
      if (process.platform === 'win32') {
        ...
        console.error(chalk.gray('  • Use sudo: sudo spoofd <command>'))
      }
    }
  }
...
async function init () {
  if (cmd === 'version' || argv.version) {
    version()
  } else if (cmd === 'help' || argv.help || argv.h) {
    help()
    ...
    // Bare `spoofd` is a request for help, and succeeds.
    help()
  } else {
    // An unrecognized command is a usage error.
    console.error(chalk.red(`✗ Unknown command: ${cmd}`))
    console.error(chalk.yellow('Run `spoofd help` to see available commands.\n'))
    process.exitCode = 1
    help()
  }
}
...
function help () {
  ...
  if (platform === 'win32') {
    example = '      spoofd randomize "Ethernet"'
    note = '\n    Note: On Windows, run PowerShell or CMD as Administrator.'
  } else if (platform === 'darwin') {
    example = '      spoofd randomize en0'
    note = '\n    Note: On macOS/Linux, use sudo for MAC address changes.'
  } else {
    example = '      sudo spoofd randomize eth0'
    note = '\n    Note: On Linux, use sudo for MAC address changes.'
  }

  const message = stripIndent`
    spoofd - MAC address and DHCPv6 DUID spoofing, on Windows, macOS and Linux

    Example (randomize MAC address):
${example}${note}

    Usage:
      spoofd list [--wifi]                     List available devices.
      spoofd set <mac> <devices>...            Set device MAC address.
      spoofd randomize [--local] <devices>...  Set device MAC address randomly.
      spoofd reset <devices>...                Reset device MAC address to default.
      spoofd normalize <mac>                   Given a MAC address, normalize it.
      spoofd help                              Shows this help message.
      spoofd version | --version | -v          Show package version.
    ...
    Commands:
      ...
      vendor <mac>                      Look up the vendor behind a MAC address.
      vendor --update                   Download the full IEEE OUI registry and cache it.
      ...
      duid <command>                    DHCPv6 DUID spoofing commands (see: spoofd duid help).
    ...
    Options:
      ...
      --update        (vendor only) Refresh the cached IEEE OUI registry.
...
async function set (mac, devices) {
  if (!mac) {
    throw new Error('MAC address is required. Usage: spoofd set <mac> <device>')
  }

  if (!devices || devices.length === 0) {
    throw new Error('Device name is required. Usage: spoofd set <mac> <device>')
  }
...
function normalize (mac) {
  if (!mac) {
    throw new Error('MAC address is required. Usage: spoofd normalize <mac>')
  }
...
async function randomize (devices) {
  if (!devices || devices.length === 0) {
    throw new Error('Device name is required. Usage: spoofd randomize <device>')
  }
...
async function reset (devices) {
  if (!devices || devices.length === 0) {
    throw new Error('Device name is required. Usage: spoofd reset <device>')
  }
...
function info (device) {
  if (!device) {
    throw new Error('Device name is required. Usage: spoofd info <device>')
  }
...
function validate (mac) {
  if (!mac) {
    throw new Error('MAC address is required. Usage: spoofd validate <mac>')
  }
  ...
  // An invalid address has to fail the process, not just print a verdict --
  // `spoofd validate "$MAC" && use_it` must be safe to write in a script.
  if (!isValid) {
    process.exitCode = 1
  }
...
async function vendor (mac) {
  if (argv.update) {
    await updateVendorRegistry()
    if (!mac) return
  }

  if (!mac) {
    throw new Error('MAC address is required. Usage: spoofd vendor <mac>')
  }
  ...
  if (vendorInfo.vendor === 'Unknown') {
    if (oui.registryStatus().available) {
      console.log(chalk.gray('\nNot in the IEEE registry. Locally administered addresses -- including'))
      console.log(chalk.gray('every address `spoofd randomize` generates -- have no vendor by design.'))
    } else {
      console.log(chalk.gray('\nNot in the bundled vendor table, which only covers common prefixes.'))
      console.log(chalk.gray('Run `spoofd vendor --update` to fetch the full IEEE registry, or this may'))
      console.log(chalk.gray('simply be a locally administered address, which has no vendor at all.'))
    }
  }
}

/**
 * Fetch the full IEEE OUI registry and cache it on disk. Explicit rather than
 * automatic: a lookup should never quietly reach the network.
 */
async function updateVendorRegistry () {
  showProgress('Downloading the IEEE OUI registry')
  let result
  try {
    result = await oui.updateRegistry()
  } catch (err) {
    failProgress('Could not update the vendor registry')
    throw new Error(`${err.message}. The bundled vendor table still works offline.`)
  }
  successProgress(`Cached ${result.count} vendor prefixes`)
  if (JSON_OUTPUT) {
    outputJSON({ updated: result.updated, count: result.count, path: result.path })
  } else {
...
async function batch (file) {
  if (!file) {
    throw new Error('Batch file is required. Usage: spoofd batch <file>')
  }
  ...
  for (let index = 0; index < batchConfig.length; index++) {
    ...
    try {
      if (operation.type === 'set' && operation.device && operation.mac) {
        ...
        results.push({ success: true, operation, index })
        successCount++
        ...
        results.push({ success: true, operation, mac, index })
        successCount++
      ...
      results.push({ success: false, operation, error: err.message, index })
      failCount++
    }
  }
