...
// SPOOFY_* are the pre-rename names, still honoured.
const verboseEnv = process.env.SPOOFD_VERBOSE || process.env.SPOOFY_VERBOSE
const jsonEnv = process.env.SPOOFD_JSON || process.env.SPOOFY_JSON
if (typeof verboseEnv !== 'undefined') {
  VERBOSE = verboseEnv === 'true'
}
if (typeof jsonEnv !== 'undefined') {
  JSON_OUTPUT = jsonEnv === 'true'
}
...
function checkPrivileges () {
  if (os.platform() === 'win32') {
    try {
      require('child_process').execFileSync('net', ['session'], { stdio: 'pipe' })
      return true
    } catch (e) {
      return false
    }
...
const commands = {
    ...
    switch (subcommand) {
        ...
        if (duid.hasOriginalDUID()) {
          ...
          if (!args.includes('--force')) {
            if (JSON_OUTPUT) {
              ...
              console.log()
              console.log(chalk.blue('ℹ'), 'To confirm, run: spoofd duid original clear --force')
            }
            process.exit(1)
          }
      ...
      default:
        if (JSON_OUTPUT) {
          ...
          console.log(chalk.blue('ℹ'), 'Usage: spoofd duid original [show|clear|path]')
        }
        process.exit(1)
    }
  },
  ...
  randomize (args) {
    if (!checkPrivileges()) {
      if (JSON_OUTPUT) {
        ...
        console.error(chalk.red('✗'), 'This command requires root/administrator privileges')
        console.log(chalk.blue('ℹ'), 'Try: sudo spoofd duid randomize [interface]')
      }
      process.exit(1)
    }
    ...
    try {
      ...
      if (JSON_OUTPUT) {
        ...
        console.log(chalk.blue('ℹ'), 'The original DUID has been backed up and can be restored with: spoofd duid restore')
      }
    } catch (e) {
      if (JSON_OUTPUT) {
        outputJSON({ error: e.message, success: false })
  ...
  set (args) {
    if (!checkPrivileges()) {
      if (JSON_OUTPUT) {
        ...
        console.error(chalk.red('✗'), 'This command requires root/administrator privileges')
        console.log(chalk.blue('ℹ'), 'Try: sudo spoofd duid set <duid-hex> [interface]')
      }
      process.exit(1)
    }
    ...
    if (args.length < 1) {
      if (JSON_OUTPUT) {
        ...
        console.error(chalk.red('✗'), 'Usage: spoofd duid set <duid-hex> [interface]')
        console.log(chalk.blue('ℹ'), 'Example: spoofd duid set 00:03:00:01:aa:bb:cc:dd:ee:ff en0')
      }
      process.exit(1)
    }
  ...
  reset (args) {
    if (!checkPrivileges()) {
      if (JSON_OUTPUT) {
        ...
        console.error(chalk.red('✗'), 'This command requires root/administrator privileges')
        console.log(chalk.blue('ℹ'), 'Try: sudo spoofd duid reset [interface]')
      }
      process.exit(1)
    }
  ...
  restore (args) {
    if (!checkPrivileges()) {
      if (JSON_OUTPUT) {
        ...
        console.error(chalk.red('✗'), 'This command requires root/administrator privileges')
        console.log(chalk.blue('ℹ'), 'Try: sudo spoofd duid restore [interface]')
      }
      process.exit(1)
    }
  ...
  generate (args) {
    ...
    if (JSON_OUTPUT) {
      ...
      console.log(`  sudo spoofd duid set ${duid.formatDUID(generated)} [interface]`)
      console.log()
    }
  },
  ...
  sync (args) {
    if (!checkPrivileges()) {
      if (JSON_OUTPUT) {
        ...
        console.error(chalk.red('✗'), 'This command requires root/administrator privileges')
        console.log(chalk.blue('ℹ'), 'Try: sudo spoofd duid sync <interface>')
      }
      process.exit(1)
    }
    ...
    if (args.length < 1) {
      if (JSON_OUTPUT) {
        ...
        console.error(chalk.red('✗'), 'Usage: spoofd duid sync <interface> [--type=<type>]')
        console.log(chalk.blue('ℹ'), 'Example: sudo spoofd duid sync en0 --type=LLT')
      }
      process.exit(1)
    }
  ...
  help () {
    if (JSON_OUTPUT) {
      ...
      console.log(`
${chalk.bold('spoofd duid')} - DHCPv6 DUID spoofing utility
...
${chalk.bold('EXAMPLES:')}
  spoofd duid list
  sudo spoofd duid randomize en0
  sudo spoofd duid randomize eth0 --type=LLT
  sudo spoofd duid set 00:03:00:01:aa:bb:cc:dd:ee:ff
  sudo spoofd duid sync en0                   # Sync DUID to current MAC
  sudo spoofd duid restore                    # Restore original DUID
  sudo spoofd duid reset                      # Generate new system DUID
  spoofd duid history                         # View all DUID changes
  spoofd duid history en0                     # View DUID changes for en0
  spoofd duid generate --type=UUID
  spoofd duid original show
...
${chalk.bold('TYPICAL WORKFLOW:')}
  ${chalk.cyan('sudo spoofd randomize en0')}      # Spoof MAC first
  ${chalk.cyan('sudo spoofd duid sync en0')}      # Sync DUID to match spoofed MAC
...
${chalk.bold('ORIGINAL DUID STORAGE:')}
  ...
  This allows you to restore it later with 'spoofd duid restore'.
...
/**
 * Integration function to add DUID commands to existing spoofd CLI
 */
