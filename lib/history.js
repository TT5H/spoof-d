...
const HISTORY_FILE = resolveStatePath(
  path.join(os.homedir(), '.spoofd_history.json'),
  path.join(os.homedir(), '.spoofy_history.json')
)
...
function saveHistory (history) {
  ...
  try {
    ...
    // Clean up the actual temp file if write or rename failed
    try {
    ...
    if (process.env.DEBUG || process.env.SPOOFD_VERBOSE || process.env.SPOOFY_VERBOSE) {
