...
async function isNetworkManagerPresent () {
  ...
  try {
    ...
    const running = output.trim().toLowerCase() === 'yes' || output.trim().toLowerCase() === 'running'
    return {
      present: true,
      running,
      method: 'nmcli'
...
// NetworkManager is Linux-only. Everywhere else, export stubs that report
// "not present" instead of shelling out to a binary that cannot exist.
