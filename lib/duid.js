...
const os = require('os')
const { sanitizeInterfaceName, escapePowerShell, sleepSync, retry } = require('./utils')
  ...
  try {
    if (platform === 'darwin' || platform === 'linux') {
      ...
      const output = execSync(
        "powershell -Command \"Get-NetAdapter -Name '" + escaped + "' | Select-Object -ExpandProperty MacAddress\"",
        { encoding: 'utf8' }
      ).trim()
...
const macos = {
  ...
  setDUID (duid, iface = null) {
    this.backupOriginal()
    var safeIface = iface ? sanitizeInterfaceName(iface) : null
    if (safeIface) {
    ...
    try {
      var duidDir = path.dirname(this.DUID_PATH)
      if (fs.existsSync(duidDir)) {
        ...
        var leasesDir = path.join(duidDir, 'leases')
    ...
    var duidDirPath = path.dirname(this.DUID_PATH)
  ...
  restoreDUID (iface = null) {
    ...
    try {
      ...
      var leasesDir = path.join(path.dirname(this.DUID_PATH), 'leases')
      if (fs.existsSync(leasesDir)) {
        fs.readdirSync(leasesDir).forEach(function (f) {
          try { fs.unlinkSync(path.join(leasesDir, f)) } catch (e) { /* ignore */ }
        })
  ...
  resetDUID (iface = null) {
    var safeIface = iface ? sanitizeInterfaceName(iface) : null
    var safeTarget = safeIface ? (this.getHardwarePort(safeIface) || safeIface) : null
  ...
  getHardwarePort (device) {
    try {
      ...
      var safeDevice = sanitizeInterfaceName(device)
  ...
  _setV6Off (ifaceOrPort) {
    try {
      ...
      var hwPort = this.getHardwarePort(ifaceOrPort)
      if (hwPort) {
        try {
...
const linux = {
        ...
        if (safeIface) {
          ...
          try {
            ...
            if (connName) {
              ...
              var safeConn = sanitizeInterfaceName(connName)
  ...
  restoreDUID (iface = null) {
    ...
    switch (client) {
      ...
      case 'networkmanager': {
        if (safeIface) {
          try {
            var rConnName = execFileSync('nmcli', ['-g', 'GENERAL.CONNECTION', 'device', 'show', safeIface], {
              encoding: 'utf8'
            }).trim()

            if (rConnName) {
              var safeRConn = sanitizeInterfaceName(rConnName)
  ...
  resetDUID (iface = null) {
    ...
    switch (client) {
      ...
      case 'dhclient': {
        if (fs.existsSync(this.DHCLIENT_CONF)) {
          var dhResetContent = fs.readFileSync(this.DHCLIENT_CONF, 'utf8')
      ...
      case 'networkmanager': {
        if (safeIface) {
          try {
            var resetConnName = execFileSync('nmcli', ['-g', 'GENERAL.CONNECTION', 'device', 'show', safeIface], {
              encoding: 'utf8'
            }).trim()

            if (resetConnName) {
              var safeResetConn = sanitizeInterfaceName(resetConnName)
...
const windows = {
  ...
  _restartIPv6 (iface) {
    if (!iface) return
    var safeIface = sanitizeInterfaceName(iface)
    try {
      ...
      var escaped = escapePowerShell(safeIface)
      try {
        execSync(
          "powershell -Command \"Disable-NetAdapterBinding -Name '" + escaped + "' -ComponentID ms_tcpip6; Enable-NetAdapterBinding -Name '" + escaped + "' -ComponentID ms_tcpip6\"",
          { stdio: 'pipe' }
        )
...
function setDUID (duid, iface = null, verify = true) {
  var duidBuf = Buffer.isBuffer(duid) ? duid : hexToDuid(duid)
  ...
  if (verify) {
    ...
    if (!newDuid) {
      ...
      var errMsg = 'DUID change verification failed. ' +
    ...
    var expectedHex = duidBuf.toString('hex').toLowerCase()
    var actualHex = newDuid.toString('hex').toLowerCase()
